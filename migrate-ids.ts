 ...
 * 
 * Usage: npx ts-node migrate-ids.ts
 */
...
class IDMigrationTool {
  ...
  private backupDir = './backup-before-migration';
  ...
  private async processDirectory(dir: string, extensions: string[]): Promise<void> {
    try {
  ...
  private processCSS(content: string): string {
    let updated = content;
    let changes = 0;
  ...
  private processJavaScript(content: string): string {
    let updated = content;
    let changes = 0;
...
export default IDMigrationTool;

...
import { ResponsiveConfig, DeviceInfo, ScalingInfo } from '../types';
...
export class ResponsiveCanvas {
  ...
  private handleResize(): void {
    ...
    const devicePixelRatio = this.config.devicePixelRatio ? deviceInfo.devicePixelRatio : 1;
   ...
   */
  private updateCanvasDimensions(width: number, height: number, devicePixelRatio: number): void {
    // Set CSS size
  ...
  private notifyCallbacks(deviceInfo: DeviceInfo): void {
    ...
    this.resizeCallbacks.forEach(callback => {
      try {
        callback(scalingInfo.scaleX || scalingInfo.widthScale, scalingInfo.scaleY || scalingInfo.heightScale, deviceInfo);
      } catch (error) {
    ...
    if (window.game?.onResize) {
      window.game.onResize(scalingInfo.scaleX || scalingInfo.widthScale, scalingInfo.scaleY || scalingInfo.heightScale, deviceInfo.isDesktop);
    }
  }
   ...
   */
  public static setup(canvasId: string, config?: Partial<ResponsiveConfig>): ResponsiveCanvas {
